...
mkdir "$HOME/.MiniPass"
echo "Done!"
echo "$PW" | gpg --batch --quiet --no-tty --passphrase-fd 0 --pinentry-mode loopback -c - > "$HOME/.MiniPass/.MiniPass.txt.gpg"
...
echo
if gpg --batch --quiet --no-tty --passphrase "$PW" --pinentry-mode loopback -d "$HOME/.MiniPass/.MiniPass.txt.gpg" > /dev/null 2>&1; then
echo -e "$NU\n$NP" | gpg --batch --quiet --no-tty --passphrase-fd 0 --pinentry-mode loopback -c - > "$HOME/.MiniPass/.$NN.txt.gpg"
